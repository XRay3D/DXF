...
{
    //    qDebug(Q_FUNC_INFO);
    //QPen p(Qt::black, constantWidth, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin);
    QPen p(Qt::red, 0.0);
    if (bulge != 0.0) {
...

        {
            ...
            //            scene->addPath(pp, QPen(Qt::red, constantWidth), Qt::NoBrush);

            auto item = scene->addEllipse(QRectF(o + QPointF(r, r), o - QPointF(r, r)), QPen(Qt::red, constantWidth), Qt::NoBrush);
            item->setStartAngle(oa.angle() * 16);
            item->setSpanAngle(oa.angleTo(ob) * 16);
        }
    } else {
